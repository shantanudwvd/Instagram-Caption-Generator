...
class CaptionLearningService {
        ...
        while (!connected && retries < maxRetries) {
            try {
                ...
                if (retries >= maxRetries) {
                    logger.error(
                        'Max MongoDB connection retries reached. Giving up.'
                    );
                    throw new Error(
    ...
    async _setupIndexes() {
        try {
            ...
            await db
                .collection(this.captionCollection)
                .createIndex({ createdAt: -1 });
            await db
                .collection(this.captionCollection)
                .createIndex({ avgRating: -1 });
            await db
                .collection(this.captionCollection)
                .createIndex({ feedbackCount: -1 });
            await db
                .collection(this.captionCollection)
            ...
            await db
                .collection(this.fineTuningCollection)
                .createIndex({ createdAt: -1 });
            await db
                .collection(this.fineTuningCollection)
    ...
    async storeCaption(captionData) {
        ...
        try {
            ...
            const sanitizedData = {
                ...
                imageAnalysis: this._sanitizeText(
                    captionData.imageAnalysis || ''
                ),
                imageFeatures: this._sanitizeObject(
                    captionData.imageFeatures || null
                ),
                imageUrl: captionData.imageUrl || null,
                songAnalysis: this._sanitizeObject(
                    captionData.songAnalysis || {}
                ),
                songFeatures: this._sanitizeObject(
                    captionData.songFeatures || null
                ),
    ...
    async recordFeedback(captionId, feedback) {
        ...
        try {
            ...
            if (
                captionDoc.userId &&
                captionDoc.userId.toString() !== feedback.userId
            ) {
            ...
            const totalRating =
                captionDoc.avgRating * captionDoc.feedbackCount +
                feedback.rating;
            ...
            logger.info(
                `Recorded feedback for caption ${captionId}: rating ${feedback.rating}`
            );
            return true;
        } catch (error) {
    ...
    async generateTrainingData(options = {}) {
        ...
        try {
            ...
            if (highQualityCaptions.length === 0) {
                logger.warn(
                    'No captions meet the quality criteria for training data'
                );
                return [];
            }
            ...
            for (const caption of highQualityCaptions) {
                ...
                if (captionUserId) {
                    if (userProfileCache.has(captionUserId)) {
                        ...
                        try {
                            styleProfile = await this.getUserPreferenceProfile(
                                captionUserId,
                                { forceRefresh: false }
                            );
                            ...
                            logger.warn(
                                'Unable to load user style profile for training data',
                                {
                ...
                trainingData.push({
                    context: {
                        ...
                        relationshipAnalysis:
                            caption.relationshipAnalysis || null,
            ...
            throw new Error(
                `Failed to generate training data: ${error.message}`
            );
        }
    }
    ...
    async finetuneModel(options = {}) {
        ...
        try {
            ...
            const modelSuffix = `caption-generator-${
                new Date().toISOString().split('T')[0]
            }`;

            logger.info(
                `Starting fine-tuning job with ${trainingData.length} examples...`
            );
    ...
    async checkFineTuningStatus(jobId) {
        ...
        try {
            ...
            throw new Error(
                `Failed to check fine-tuning status: ${error.message}`
            );
        }
    }
    ...
    async getLatestFineTunedModel(options = {}) {
        ...
        try {
            ...
            if (completedJobs.length > 0) {
                logger.info(
                    `Found latest fine-tuned model: ${completedJobs[0].fineTunedModel}`
                );
    ...
    async getDashboardStats(userId) {
        ...
        try {
            ...
            const totalCaptions = await captionColl.countDocuments(
                userCaptionMatch
            );
            ...
            const fineTuningStats = {
                ...
                succeeded: await fineTuneColl.countDocuments({
                    status: 'succeeded',
                }),
    ...
    async getUserPreferenceProfile(userId, options = {}) {
        ...
        try {
            ...
            if (!forceRefresh) {
                const existing = await preferenceColl.findOne({
                    userId: userObjectId,
                });
                if (existing) {
            ...
            const scoredCaptions = [...captions].sort((a, b) => {
                const scoreA =
                    (a.avgRating || 0) + (a.feedbackCount || 0) * 0.1;
                const scoreB =
                    (b.avgRating || 0) + (b.feedbackCount || 0) * 0.1;
                if (scoreA === scoreB) {
...
- Feedback: ${
            c.feedbackSample
                .map(
                    (f) =>
                        `rating ${f.rating}${
                            f.comments ? `, comment: ${f.comments}` : ''
                        }${f.userEdits ? `, edits: ${f.userEdits}` : ''}`
                )
                .join(' | ') || 'none'
        }
`
...
`,
                    ...
                    logger.warn(
                        'Failed to build AI preference profile, using heuristics',
            ...
            const profile = {
                ...
                preferredOptions: {
                    tone:
                        aiProfile?.preferredOptions?.tone ||
                        preferredOptions.tone,
                    length:
                        aiProfile?.preferredOptions?.length ||
                        preferredOptions.length,
                    emoji:
                        aiProfile?.preferredOptions?.emoji ||
                        preferredOptions.emoji,
                    hashtags:
                        aiProfile?.preferredOptions?.hashtags ||
                        preferredOptions.hashtags,
                    language:
                        aiProfile?.preferredOptions?.language ||
                        preferredOptions.language,
                },
                examplePhrases:
                    aiProfile?.examplePhrases ||
                    topCaptions.map((c) => c.caption).slice(0, 3),
            ...
            const profileDoc = {
                ...
                stylePrinciples: this._sanitizeArray(
                    profile.stylePrinciples || []
                ),
                dos: this._sanitizeArray(profile.dos || []),
                donts: this._sanitizeArray(profile.donts || []),
                preferredOptions: this._sanitizeObject(
                    profile.preferredOptions || {}
                ),
                examplePhrases: this._sanitizeArray(
...
- Thematic Connections: ${
                  item.context.relationshipAnalysis.thematicConnections?.join(
                      ', '
                  ) || 'Various'
              }
- Emotional Resonance: ${
                  item.context.relationshipAnalysis.emotionalResonance ||
                  'Cohesive emotional experience'
              }
- Integration Suggestions: ${
                  item.context.relationshipAnalysis.integrationSuggestions?.join(
                      '; '
                  ) || 'Weave elements naturally together'
              }
`
            : '';
...
- Style principles: ${
                  (item.context.styleProfile.stylePrinciples || []).join(
                      '; '
                  ) || 'Stay specific; keep it natural'
              }
- Dos: ${
                  (item.context.styleProfile.dos || []).join('; ') ||
                  'Lean into concrete details'
              }
- Donts: ${
                  (item.context.styleProfile.donts || []).join('; ') ||
                  'Avoid generic filler'
              }
- Preferred options: ${JSON.stringify(
                  item.context.styleProfile.preferredOptions || {}
              )}
- Example snippets: ${
                  (item.context.styleProfile.examplePhrases || [])
                      .slice(0, 3)
                      .join(' | ') || 'n/a'
              }
...
SONG:
"${item.context.songAnalysis?.name || 'Unknown'}" by ${
            item.context.songAnalysis?.artist || 'Unknown Artist'
        }
...
${
    item.context.userContext
        ? `ADDITIONAL CONTEXT: ${item.context.userContext}`
        : ''
}
`;
    }
    ...
    _collectOptionStats(captions) {
        ...
        captions.forEach((caption) => {
            ...
            ['tone', 'length', 'emoji', 'hashtags', 'language'].forEach(
                (key) => {
    ...
    async deleteCaption(captionId) {
        ...
        try {
            ...
            const db = this.getDb();
            const collection = db.collection(this.captionCollection);
            const captionData = collection.find({
                captionId,
                status: 'active',
            });
            if (!captionData) {
