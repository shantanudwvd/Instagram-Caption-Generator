...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        ...
        if (file.fieldname === 'audio') {
            ...
            const hasSupportedMime = Boolean(
                AUDIO_MIME_EXTENSIONS[file.mimetype]
            );

            if (!hasSupportedExt && !hasSupportedMime) {
                return cb(
                    new Error('Unsupported audio format for transcription')
                );
            }
    ...
    async (req, res) => {
        try {
            ...
            const imageAnalysisText =
                imageAnalysisResult.text || imageAnalysisResult;
            ...
            try {
                userStyleProfile =
                    await captionLearningService.getUserPreferenceProfile(
                        req.user.id
                    );
            ...
            const customization = {
                tone:
                    tone ||
                    userStyleProfile?.preferredOptions?.tone ||
                    'casual',
                length:
                    length ||
                    userStyleProfile?.preferredOptions?.length ||
                    'medium',
                language:
                    language ||
                    userStyleProfile?.preferredOptions?.language ||
                    'english',
                emoji:
                    emoji ||
                    userStyleProfile?.preferredOptions?.emoji ||
                    'moderate',
                hashtags:
                    hashtags ||
                    ...
                    if (imageFeatures && songFeatures) {
                        try {
                            relationshipAnalysis =
                                await analyzeImageSongRelationship(
                                    imageFeatures,
                                    songFeatures,
                                    imageAnalysisText,
                                    songAnalysis
                            ...
                            logger.error(
                                'Error analyzing image-song relationship',
                                {
            ...
            try {
                ...
                if (imageUrl) {
                    try {
                        ...
                        logger.error(
                            'Error cleaning up image after failed storage',
                            {
...
router.post('/get-recommendations', async (req, res) => {
    try {
        ...
        if (!imageAnalysis) {
            return res
                .status(400)
                .json({ error: 'Image analysis is required' });
        }
        ...
        try {
            ...
            logger.warn(
                'Using default queries for recommendations due to error',
                {
                    userId: req.user?.id,
                }
            );
            ...
            const defaultRecommendations =
                await recommendationService.searchTracksWithQueries(
                    defaultQueries
                );
            res.json({
...
`,
        ...
        const extractedFeatures = JSON.parse(
...
async function generateCaption(
    ...
    const lengthMap = {
        'very-short': {
            description: 'very brief, just 1 sentence',
            maxWords: 20,
        },
...
`,
        ...
        logger.debug('Image-song relationship analyzed', {
            relationshipAnalysis,
        });
        ...
        return {
            ...
            thematicConnections: [
                'Both elements share a similar mood and energy',
            ],
            emotionalResonance:
                'The image and song create a cohesive emotional experience',
...
async function analyzeSong(trackId) {
    try {
        ...
        return {
            ...
            album: 'Unknown Album',
            description:
                'A track that could complement the mood of your image.',
            features: {
                energy: 0.5,
                mood: 'neutral',
                tempo: 'moderate',
